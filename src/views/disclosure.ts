...
export function disclosureView(siteName: string): string {
  ...
  <div class="page-header">
    <div class="wrap">
      ...
      <p>How ${brandText(siteName)} makes money, in plain terms.</p>
    </div>
  </div>
  <section class="section wrap prose">
    <p>${brandText(siteName)} participates in affiliate marketing programs, including the Amazon Associates Program, which means we may earn a commission when you click a link on this site and make a qualifying purchase — at no additional cost to you.</p>
    ...
    <ul>
      ...
      <li>All purchases, payments, shipping and returns are handled entirely by the retailer (e.g. Amazon), not by ${brandText(siteName)}.</li>
    </ul>
